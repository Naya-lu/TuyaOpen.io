...
`tkl_ota.c` 固件 OTA（OTA 即 Over-the-Air，是一种通过无线通讯进行固件升级的技术），它包含处理固件升级流程的各个阶段：获取固件升级能力、OTA 启动通知、OTA 数据处理、OTA 结束通知。该文件由涂鸦操作系统自动生成，并允许开发者在提供的用户定义区域内添加自己的实现。
...
- 参数：`reset`，输入参数，表示 OTA 结束后是否需要重置设备。

- 返回值：返回值为 `OPRT_OK` 表示操作成功，如果校验失败或其他原因导致失败，则返回错误代码。详细错误代码请参考 `tuya_error_code.h`。
